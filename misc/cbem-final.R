...
cbem_plot <- function(state, age_group_to_filter) {
  ...
  cbem |>
    ...
    geom_hline(yintercept = state_avg, linetype = "dashed", color = "grey60") +
    geom_chicklet() +
...
alabama_under_18 <-
  cbem_plot(state = "Alabama", age_group_to_filter = "Under 18")

alabama_under_25 <-
  cbem_plot(state = "Alabama", age_group_to_filter = "Under 25")
